module.exports = function(io) {
    ...
    chess_socket.on('connection', function (socket) {
		socket.on('join', function(room) {
			...
			socket.join(room);
			socket.emit('pseudo', socket.request.user.username)
			
			console.log( socket.request.user.username + ' connected to the chess lobby n.' + room + '.');
			...
			if (game == null) {
				...
				is_ready( game , chess_socket, room );
			...
			socket.on('start_game', function() {			
				...
				if ( game_started ) {
					...
					chess_socket.in(room).emit('start', teams);
				};
			});
			...
			socket.on('move', function(data) {
				...
				if (move_result[0] != "forbidden") {
					...
					if ( move_ai != null ) {
						chess_socket.in(room).emit( move_ai[0], move_ai[1] );
					};
				};
			});
			...
			socket.on('reset', function () {
				...
				chess_socket.in(room).emit('reset', reset);
