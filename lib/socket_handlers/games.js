		...
		socket.on('join', function(room) {
			...
			socket.on('move', function(data) {
				...
				if ( game.get_status != 'ongoing' ) {
					console.log(socket.player.NAME + " a voulu jouer sans que la partie soit demarrée");
					socket.emit('forbidden', 'The game has not started yet');
				} else {
					console.log("Turn : " + game.process().turn());  // NEED TO USE THIS NOTATION
					
					console.log( socket.player.get_team() );
					if ( game.process().can_play( socket.player.get_team() ) ) {
						...
						if (move === null)  {
							...
							// TODO
							// Gerer la fin de partie (suivant les trucs possibles)
			...
			socket.on('disconnect', function () {
				...
				*/
				if ( game.get_status != 'ready' ) {
					// First case, in the lobby, without plaiying
					
					
				} else {
					//socket.broadcast.to(room).emit('client_disconnected', socket.request.user.username);
					//game.get_users[1].socket() = null;
					//game.get_users[1].NAME = "";
					//game.get_users[1].get_status = "open";
